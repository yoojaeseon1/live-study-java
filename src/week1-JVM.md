...
- 좀 더 확인 필요
  ...
  - 초기 JVM은 인터프리터(한줄씩 해석해고 실행)만 실행했었기 때문에 속도가 느렸다.
...
###### 어플리케이션 클래스로더(java 9부터 확장 클래스로더)
...
- Verification(확인) : 로딩된 클래스의 바이트 구조가 올바른지 검사(보안)

- Preparation(준비)

  - 클래스, 인터페이스의 static 필드를 생성하고 기본 값으로 초기화

  - 클래스 변수와 기본 값에 필요한 메모리를 메서드 영역에 있는 실제 레퍼런스로 교체(위와 같은 말)
